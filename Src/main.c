...

            {
                ...
                case 2:
                {
                    if (cs_ph == 0)
                    {
                        ...
                        if (d_gray == 0x00 && (d_er < 0 ? -d_er : d_er) < 3)
                        {
                    ...
                    else  // cs_ph != 0 -> turning phase
                    {
                        ...
                        else
                        {
                            ...
                            // Exit when yaw changed ~85 degrees (850 units, yaw*10)
                            // Minimum 200ms (10 cycles) to ensure at least some rotation
                            // Maximum 60 cycles (1200ms) as safety timeout
                            int16_t yaw_delta = d_yaw - yaw_start;
                            if (yaw_delta < -30000) yaw_delta += 3600;  // wrap around 360 deg
                            if (yaw_delta > 30000) yaw_delta -= 3600;
                            if ((cs_tm >= 10 && yaw_delta <= -850) || cs_tm >= 60)
                            {
                                cs_ph = 0; cs_tm = 0;
                                integralL = 0.0f; integralR = 0.0f;
                                Motor_GetLeftPulses(); Motor_GetRightPulses();
